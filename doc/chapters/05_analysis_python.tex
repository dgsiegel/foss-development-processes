...
\noindent Python is an object oriented, interpreted programming language
...
it is available for all major platforms. Python comes with an interpreter and
an extensive standard library. It supports many programming paradigms, such as
...
around 1989 at the \textsmaller{CWI} Institute in the Netherlands
\cite{Venners2003}. Python was a successor to the \textsmaller{ABC} programming
...
\begin{enumerate}
  ...
  \item \spacedlowsmallcaps{Standards Track PEP} The most common form of
  ...
  \item \spacedlowsmallcaps{Informational PEP} Unlike the Standards Track
    \textsmaller{PEP}, the Informational \textsmaller{PEP} does not propose a
  ...
  \item \spacedlowsmallcaps{Process PEP} This kind of \ac{PEP} is quite like
    the Standards Track \textsmaller{PEP}, except that it applies to other
    ...
    others are mostly Process \textsmaller{PEPs}. Additionally any \acp{PEP}
    which describe other \acp{PEP}, for example how a \ac{PEP} should look
    like, will be defined as a Process \textsmaller{PEPs}.
...
Mostly for Informational \textsmaller{PEPs}, subsequent versions can replace a
\ac{PEP}. In this case the original \ac{PEP} would be marked as Replaced and
superseded by the new \ac{PEP}.

\paragraph{Active}

Informational and Process \textsmaller{PEPs} can also be marked as Active, if they will never
